...
import com.steve.dataconsolidate.DCHttpRequestWrapper;
...
public final class CORSFilter implements Filter {
    ...
    public void doFilter(final ServletRequest servletRequest,
        ...
        HttpServletRequest requestOrg = (HttpServletRequest) servletRequest;
        HttpServletResponse response = (HttpServletResponse) servletResponse;

        // Construct wrapped http request handler
        DCHttpRequestWrapper request = new DCHttpRequestWrapper((HttpServletRequest) requestOrg);
        
        // If the payload's content type is "application/json", log it (requires TRACE level)
        if( request != null){
        	
        	request.logRequestURL();
        	
        	if( (request.getContentType() != null) && (request.getContentType().equalsIgnoreCase("application/json"))){
        		request.logTextPayload();
        	}
